...
const StoryCircle = React.memo(({ story, isSelf, selfUser }: { story?: Story; isSelf?: boolean, selfUser?: User | null }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-2 flex-shrink-0 cursor-pointer">
      <div className={cn("h-16 w-16 rounded-full p-0.5 flex items-center justify-center transition-all", story.isSeen ? 'bg-muted' : 'bg-gradient-to-tr from-primary to-secondary')}>
                 ...
                 <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 space-y-3">
                      <textarea
                        ...
                        className="w-full resize-none rounded-xl bg-background/60 border border-border/40 px-4 py-3 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all"
                        rows={focused ? 3 : 1}
                      />
                      <AnimatePresence>
                        {focused && (
...
const PostCard = React.memo(({ post }: { post: Post }) => {
    ...
    return (
        <Card className="group overflow-hidden border-border/60 transition-shadow hover:shadow-card-hover">
        ...
        <Card className="h-full flex flex-col transition-all hover:shadow-lg hover:-translate-y-1">
            <CardContent className="p-6 flex-grow flex flex-col">
                <div className="flex justify-between items-start">
                    <div className="p-3 bg-muted rounded-lg text-4xl">{circle.icon}</div>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <Card className="border-dashed flex flex-col items-center justify-center text-center p-6 min-h-[300px] cursor-pointer hover:border-primary hover:text-primary transition-colors">
                    <div className="p-4 border-2 border-dashed rounded-full"><Plus className="h-8 w-8" /></div>
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <main className="col-span-1 lg:col-span-3 space-y-4">
                 {discussions.map(d => (
                    <Card key={d.id} className={cn("transition-colors hover:bg-muted/50", d.isPinned && "border-amber-500/50 bg-amber-500/5 dark:bg-amber-500/10")}>
